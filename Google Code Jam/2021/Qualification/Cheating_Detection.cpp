...
int T, P, wa, answer, n = 100, m = 10000;
char s[N][M];
...
const int THRESHOLD_SUSPICION = 55;
const double THRESHOLD_PROB = 0.8;
struct Player {
    int id;
    int score;
    int suspicion;
...
int getCheater2() {
    FOR(i, 1, n) {
        p[i].id = i;
        p[i].score = 0;
        FOR(j, 1, m) p[i].score += s[i][j] == '1';
        p[i].skill = -3. + 6. * p[i].score / n;
    }

    FOR(j, 1, m) {
        q[j].id = j;
        q[j].wrong = 0;
        FOR(i, 1, n) q[j].wrong += s[i][j] == '0';
        q[j].difficulty = -3. + 6. * q[j].wrong / m;
    }
    ...
    FOR(i, 2, n) {
        p[i].suspicion = 0;
        FOR(j, 1, m) {
            if (s[i][j] == '0' && sigmoid(p[i].skill - q[j].difficulty) >= THRESHOLD_PROB) {
                ++p[i].suspicion;
            }
        }
    }
    sort(p + 2, p + 1 + n, [] (Player p1, Player p2) {
        return p1.suspicion > p2.suspicion;
    });
    if (p[2].suspicion - p[3].suspicion >= THRESHOLD_SUSPICION) {
        return p[2].id;
    }
...
void solve(int Case) { printf("Case #%d: ", Case);
    ...
    // int cheater = getCheater1();
    int cheater = getCheater2();

    pln(cheater);

#ifdef BACKLIGHT
    rd(answer); ++answer;
    debug(cheater, answer);

    if (cheater != answer) ++wa;
#endif
}
