...
int dp[N][N], sl[N][N], su[N][N], sd[N + N][N];
void solve(int Case) { // printf("Case #%d: ", Case);
    ...
    dp[1][1] = sl[1][1] = su[1][1] = sd[1- 1 + N][1] = 1;
    FOR(i, 2, n) {
        int j = 1;
        if (s[i][j] == '#') {
            dp[i][j] = 0;
            ...
            add(dp[i][j], sl[i][j - 1]);
            add(dp[i][j], su[i - 1][j]);
            add(dp[i][j], sd[i - j + N][i - 1]);

            add(sl[i][j], sl[i][j - 1]); add(sl[i][j], dp[i][j]);
            add(su[i][j], su[i - 1][j]); add(su[i][j], dp[i][j]);
            sd[i - j + N][i] = dp[i][j];
        }
    }
    ...
    FOR(i, 2, n) {
        FOR(j, 2, m) {
            if (s[i][j] == '#') {
                ...
                add(sl[i][j], sl[i][j - 1]); add(sl[i][j], dp[i][j]);
                add(su[i][j], su[i - 1][j]); add(su[i][j], dp[i][j]);
                add(sd[i - j + N][i], sd[i - j + N][i - 1]); add(sd[i - j + N][i], dp[i][j]);
            }
        }
    }
