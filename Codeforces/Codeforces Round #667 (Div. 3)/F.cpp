...
void solve(int Case) { // printf("Case #%d: ", Case);
    read(n, k);
    read_str(s); read_str(t);

    FOR(i, 0, n) FOR(j, 0, k) FOR(c, 0, i) dp[i][j][c] = -INF;

    dp[0][0][0] = 0;
    FOR(i, 0, n - 1) FOR(j, 0, k) FOR(c, 0, i) {
        if (dp[i][j][c] < 0) continue;
        updMax(dp[i + 1][j + (s[i] != t[0])][c + 1], dp[i][j][c]);
        updMax(dp[i + 1][j + (s[i] != t[1])][c + (t[0] == t[1])], dp[i][j][c] + c);
        updMax(dp[i + 1][j][c], dp[i][j][c]);
    }

    int ans = INT_MIN;
    FOR(j, 0, k) FOR(c, 0, n) updMax(ans, dp[n][j][c]);
