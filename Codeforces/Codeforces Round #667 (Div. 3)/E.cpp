...
int n, k, x[N], y[N], suf[N];

int getLastSmallThanValue(int value) {
    int l = 1, r = n, mid, pos = 0;
    while(l <= r) {
        mid = (l + r) >> 1;
        if (x[mid] <= value) l = mid + 1, pos = mid;
        else r = mid - 1;
    }
    return pos;
}
...
void solve(int Case) { // printf("Case #%d: ", Case);
    ...
    FOR(i, 1, n) {
        int L = x[i], R = L + k;
        suf[i] = getLastSmallThanValue(R) - i + 1;
    }

    suf[n + 1] = 0;
    ROF(i, n, 1) suf[i] = max(suf[i + 1], suf[i]);

    // FOR(i, 1, n) cout << suf[i] << " "; cout << endl;

    int ans = 0;
    for (int i = 1; i <= n; ++i) {
        int L1 = x[i], R1 = L1 + k;
        int j = getLastSmallThanValue(R1);
        int t1 = j - i + 1;
        int t2 = suf[j + 1];
        ans = max(ans, t1 + t2);
    }
