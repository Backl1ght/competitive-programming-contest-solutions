...
void add(int x, int d) { for (; x < N; x += lb(x)) b[x] += d; }
int getsum(int x) { int r = 0; for (; x; x -= lb(x)) r += b[x]; return r; }

void solve(int Case) { // printf("Case #%d: ", Case);
    read(n);
    read_str(s + 1);
    FOR(i, 1, n) t[i] = s[i]; reverse(t + 1, t + 1 + n);
    ...
    ROF(i, n, 1) {
        ...
        // debug(from, getsum(from - 1));

        int st = from - getsum(from - 1);

        // debug(t[i], st, dst, dst - st);
