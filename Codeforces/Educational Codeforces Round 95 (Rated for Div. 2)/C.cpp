...
void solve(int Case) { // printf("Case #%d: ", Case);
    read(n); a[n + 1] = 0;
    FOR(i, 1, n) read(a[i]);
    FOR(i, 1, n) dp1[i] = dp2[i] = INF;

    FOR(i, 0, n - 1) {
        updMin(dp1[i + 1], dp2[i] + a[i + 1]);
        updMin(dp1[i + 2], dp2[i] + a[i + 1] + a[i + 2]);

        updMin(dp2[i + 2], dp1[i + 1]);
        updMin(dp2[i + 3], dp1[i + 1]);
        updMin(dp2[i + 3], dp1[i + 2]);
        updMin(dp2[i + 4], dp1[i + 2]);
    }
