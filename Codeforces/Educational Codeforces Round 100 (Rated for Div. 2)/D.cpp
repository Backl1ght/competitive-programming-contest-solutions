...
int n, m, a[N], b[N], c[N], vis[N << 1];
void solve(int Case) { // printf("Case #%d: ", Case);
    read(n); m = n << 1;
    FOR(i, 1, m) vis[i] = 0;
    FOR(i, 1, n) read(b[i]), vis[b[i]] = 1; 
    b[n + 1] = n << 1 | 1;

    int p = 0;
    FOR(i, 1, m) if (!vis[i]) a[++p] = i;
    ...
    int need_mi = 0, delta_mi = 0;
    FOR(i, 1, n) {
        delta_mi += b[i] - b[i - 1] - 1;
        if (delta_mi == 0) ++need_mi;
        else --delta_mi;
    }

    int need_ma = 0, delta_ma = 0;
    ROF(i, n, 1) {
        delta_ma += b[i + 1] - b[i] - 1;
        if (delta_ma == 0) ++need_ma;
        else --delta_ma;
    }

    int ans = n - need_mi - need_ma + 1;
