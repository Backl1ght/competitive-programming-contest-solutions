...
void solve(int Case) { // printf("Case #%d: ", Case);
    read(n);
    FOR(i, 1, n) read(t[i], x[i]);
    t[n + 1] = LLINF;

    ll ans = 0;
    ll lst = 0, nxt = 0, st = 0, ed = 0;
    FOR(i, 1, n) {
        if (t[i] >= ed) {
            st = t[i];
            ed = t[i] + abs(x[i] - nxt);
        ...
        ll l, r;
        if (lst < nxt) {
            l = min(nxt, lst + t[i] - st);
            r = min(nxt, lst + t[i + 1] - st);
        } else {
            l = max(nxt, lst - (t[i + 1] - st));
            r = max(nxt, lst - (t[i] - st));
        }

        if (x[i] >= l && x[i] <= r) ++ans;
        debug(i, l, r, lst, nxt, st, ed, ans);
    }
