...
namespace Backlight {
    ...
    // inline bool need(char c) { return (c == '.') || (c == 's') || (c == '*')  || (c == '#')  || (c == '@'); }
...
void init() {
    f[0] = 1;
    FOR(i, 1, N - 1) f[i] = mul(f[i - 1], i);
    g[N - 1] = qp(f[N - 1], MOD - 2);
    ROF(i, N - 2, 0) g[i] = mul(g[i + 1], i + 1);
}
...
void solve(int Case) { // printf("Case #%d: ", Case);3
    ...
    FOR(mi, 1, n) {
        ...
        ans = add(ans, sub(C(cnt, m), C(cnt - (c[mi] - c[mi - 1]), m)));
    }
