...
namespace ACAM {
    ...
    void getfail() {
        queue<int> q;
        fail[0] = 0;
        for (int c = 0; c < __M; ++c) if (tr[0][c]) fail[tr[0][c]] = last[tr[0][c]] = 0, q.push(tr[0][c]);
        while(!q.empty()) {
            int p = q.front(); q.pop();
            for (int c = 0; c < __M; ++c) {
                int u = tr[p][c];
    ...
    int queryMax(char* t, int n) {
        ...
        for (int i = 0; i < n; ++i) {
            ...
            p = tr[p][c];
            for (int j = p; j; j = last[j]) if (e[j]) updMax(res, (*st[j].rbegin()));
        }
        return res;
    }
} // namespace ACAM
...
void solve(int Case) { // printf("Case #%d: ", Case);
    ...
    FOR(i, 1, m) {
        ...
        if (op == 1) {
            read(id, x);
            int p = ACAM::eid[id];
            ACAM::st[p].erase(ACAM::st[p].lower_bound(a[id]));
            a[id] = x;
            ACAM::st[p].insert(x);
        } else if (op == 2) {
            read(t); len = strlen(t);
            println(ACAM::queryMax(t, len));
        }
    }
}
