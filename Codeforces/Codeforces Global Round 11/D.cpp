...
void solve(int Case) { // printf("Case #%d: ", Case);
    ...
    FOR(i, 1, n) assert(c[i] == i);

    printf("%d\n", SZ(ans));
    for (VI vi: ans) {
        printf("%d", SZ(vi));
        for (int v: vi) printf(" %d", v);
        printf("\n");
    }
}
