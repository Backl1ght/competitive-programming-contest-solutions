...
int d[N], vis[N];
void Dijkstra() {
    FOR(i, 1, n) vis[i] = 0, d[i] = INF;
    using Node = pair<int, int>;
    priority_queue<Node, vector<Node>, greater<Node>> q;
    d[1] = 0; q.push(Node(0, 1));
    while(!q.empty()) {
        auto [dis, id] = q.top(); q.pop();
...
PII nd[N];
void DP() {
    FOR(i, 1, n) nd[i] = PII(d[i], i);
    sort(nd + 1, nd + 1 + n, greater<PII>());
    FOR(i, 1, n) {
        auto [dis, id] = nd[i];
        dp[id] = dis;
        for (auto [v, w]: G[id]) {
            if (d[id] >= d[v]) umin(dp[id], d[v]);
            if (d[id] < d[v]) umin(dp[id], dp[v]);
        }
    }
}
