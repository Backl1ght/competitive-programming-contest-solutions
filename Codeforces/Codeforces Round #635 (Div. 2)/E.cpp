...
ll dp1[3005], dp2[3005];
void solve(int Case) {
    read_str(s); n = strlen(s);
    read_str(t); m = strlen(t);
    ...
    per(i, n-1, 0) {
        ...
        rep(j, 0, n - i - 1) {
            ...
            if(s[i] == t[j] || t[j] == '?') {
                if(j > 0) {
                    dp2[j+1] = (dp2[j+1] + dp1[j]) % MOD;
                }
                else {
                    int ways = min(n-i, n-m+1);
                    dp2[j+1] = (dp2[j+1] + dp1[j] * ways % MOD) % MOD;
                }
            }
            if (s[i] == t[k] || t[k] == '?') {
        ...
        rep(i, 0 , n) dp1[i] = dp2[i];
    }
    ll ans = 0;
    rep(i, 0, n) ans = (ans + dp1[i]) % MOD;
    printf("%lld\n", ans);
}
