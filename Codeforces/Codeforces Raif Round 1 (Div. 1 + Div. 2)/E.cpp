...
void solve(int Case) { // printf("Case #%d: ", Case);
    ...
    auto cut = [sq] (int x, int t) {
        ll v = x / t, r1 = x - v * t, r0 = t - r1;
        return r0 * sq(v) + r1 * sq(v + 1);
    };
    ...
    FOR(i, 1, n) {
        all += sq(a[i]);
        q.push(make_tuple(cut(a[i], 1) - cut(a[i], 2), a[i], 2));
    }
    ...
    while(k--) {
        auto u = q.top(); q.pop();
        ll c = get<0>(u);
        int x = get<1>(u), t = get<2>(u);

        all -= c;

        q.push(make_tuple(cut(x, t) - cut(x, t + 1), x, t + 1));
    }
