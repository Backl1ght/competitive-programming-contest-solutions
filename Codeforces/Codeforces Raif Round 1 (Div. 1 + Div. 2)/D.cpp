...
void solve(int Case) { // printf("Case #%d: ", Case);
    ...
    FOR(i, 1, n) {
        ...
        int pos = i;
        while(nxt[pos]) {
            ans.PB(PII(x, nxt[pos]));
            if (a[pos] == 3) ++x, ans.EB(PII(x, nxt[pos]));
            pos = nxt[pos]; vis[pos] = 1;
        }
        ++x;
    }

    println(SZ(ans));
    for (PII pii: ans) printf("%d %d\n", pii.FI, pii.SE);
