...
namespace IO {
    ...
    inline bool need(char &c) { return (c == '0') || (c == '1') || (c=='?');}
...
struct Query {
    int id, l;
    Query() {}
    Query(int _id, int _l) : id(_id), l(_l) {}
};
vector<Query> qryEndAt[N];

int c[N];
inline int lb(int x) { return x & -x; }
void add(int x, int d) { for (; x <= n; x += lb(x)) c[x] += d; }
int getsum(int x) { int r = 0; for (; x; x -= lb(x)) r += c[x]; return r; }

void solve(int Case) { // printf("Case #%d: ", Case);
    read(n, q);
    FOR(i, 1, n) read(a[i]), a[i] = i - a[i];
    FOR(i, 1, q) {
        int x, y, l, r;
        read(x, y);
        l = 1 + x; r = n - y;
        qryEndAt[r].push_back(Query(i, l));
    }
    ...
    FOR(r, 1, n) {
        ...
        for (Query qry: qryEndAt[r]) {
            ans[qry.id] = getsum(qry.l);
        }
    }
    FOR(i, 1, q) printf("%d\n", ans[i]);
}
